...
type Pipeline interface {
	...
	doPipeline(buffer buffer.ByteBuffer) buffer.ByteBuffer
...
func (s *pipelineImpl) doPipeline(buffer buffer.ByteBuffer,a buffer.Allocator) buffer.ByteBuffer {
	tran := s.decoder(buffer)
	d := &dataImpl{data: tran}
	for i := range s.pipe {
		s.pipe[i](d)
	}
	_ = s.encoder(d.data, buffer)
	return buffer
}
